...
class StatsdMeterRegistryPublishTest {
    ...
    void receiveMetricsSuccessfully(StatsdProtocol protocol) throws InterruptedException {
        ...
        DisposableChannel server = startServer(protocol, latch, 0);
    ...
    void resumeSendingMetrics_whenServerIntermittentlyFails(StatsdProtocol protocol) throws InterruptedException {
        ...
        server = startServer(protocol, latch, port);
        counter.increment(5);
        counter.increment(6);
        counter.increment(7);
        assertThat(latch.await(1, TimeUnit.SECONDS)).isTrue();
    ...
    @NotNull
    private DisposableChannel startServer(StatsdProtocol protocol, CountDownLatch latch, int port) {
        if (protocol == StatsdProtocol.UDP) {
            ...
            return TcpServer.create()
                    ...
                    .handle((in, out) ->
                            in.receive().asString()
                                    .flatMap(packet -> {
                                        IntStream.range(0, packet.split("my.counter").length - 1).forEach(i -> latch.countDown());
                                        return Flux.never();
                                    }))
